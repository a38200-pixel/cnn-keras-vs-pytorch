...
def run_sanity_check() -> None:
    ...
    print(model)
    print(f"batch={tuple(images.shape)} labels={tuple(labels.shape)} output={tuple(output.shape)} "
          f"parameters={parameters:,} finite=True model_device={next(model.parameters()).device} "
          f"input_device={images.device} output_device={output.device}")
