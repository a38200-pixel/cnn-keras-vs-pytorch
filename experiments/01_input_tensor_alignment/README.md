...
동일 이미지의 RGB pixel 값을 같게 만들고 layout만 NHWC/NCHW로 변환해 입력 처리 영향을 확인한다.
...
입력 전처리가 원인이라면 baseline 대비 absolute Macro F1 gap이 감소할 것이다.

## Changed Variable

Input Tensor 처리만 공통 PIL RGB 변환과 bilinear resize로 정렬한다.
...
수치가 같은 HWC 배열을 만들고 Keras는 NHWC, PyTorch는 NCHW로만 배치한다. 나머지는 baseline이다.
